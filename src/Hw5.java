...
// к файлу, который программа затем будет парсить.
...
//Если указанный файл не существует или указанный путь является путём к папке, а не к файлу, выводить в консоль
// информацию об этом и продолжать цикл при помощи оператора “continue”.
...
// “Это файл номер N”, где N — порядковый номер верно указанного файла, начиная с единицы.
    ...
    public static void doPars() {
        int count = 0;
        while (count >= 0) {  // можно написать сюда true при желании, но смысл будет один и тот же!
            ...
            if (isDirectory) {
                System.out.println("Указанный путь " + path + " является путём к папке, а не к файлу\n");
                continue;
            }
